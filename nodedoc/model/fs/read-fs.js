...
//异步读取文件
fs.readFile("./nodedoc/model/text.txt", "utf-8", function (err, data) {
    if (err) {
        console.log(err);
    } else {
        console.log(data);
...
//同步读取文件（多了Sync && 不支持回调）
try {
    var syncdata = fs.readFileSync("./nodedoc/model/text.txt", "utf-8");
...
fs.stat("./nodedoc/model/text.txt", function (err, stat) {
    if (err) {
        console.log(err);
    } else {
        // 是否是文件:
