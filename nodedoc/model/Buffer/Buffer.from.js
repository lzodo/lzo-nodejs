 ...
 *  字符 H 的 ASCII 十进制为 72 => 01001000 => 48 , Buffer.from("H") 得到的会是  <Buffer 48>
 *  字符 9 的 ASCII 十进制为 57 => 00111001 => 39 , 字符9和十进制9是不同的东西
 *  一个字节（8bit）表示范围 0~255 共256个数字
 */
const fs = require("fs");
...
fs.readFile(
  path.resolve(__dirname, "./Buffer.alloc.js"),
  { encoding: "utf-8" },
  (err, data) => {
    if (err) {
      console.log(err);
    }
    console.log(data, "fs.readFile"); //不传encoding默认读取到的就是Buffer,通过二进制交互
  }
);
...
fs.readFile(path.resolve(__dirname, "../testfiles/test.jpg"), (err, data) => {
  ...
  console.log(data, "fs.readFile2"); //不传encoding默认读取到的就是Buffer,通过二进制交互

  fs.writeFile(
    path.resolve(__dirname, "../testfiles/test.copy.jpg"),
    data,
    (err) => {
      if (err) {
        console.log(err);
